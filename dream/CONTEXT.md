...
dream-chrome-extension/           # Chrome 插件（独立目录，与 dream/ 同级）
├── manifest.json                 # MV3 配置，host_permissions: localhost:45678
├── background.js                 # Service Worker：点击图标 → POST /favorite
├── icons/                        # 插件图标 PNG（16/32/48/128）
└── README.md
```
...
| 收藏 | `stores/favorite.ts` | `favorite/FavoriteView.vue` | 链接/名言两种类型，置顶，标签，分类导航，Grid卡片，刷新按钮 |
