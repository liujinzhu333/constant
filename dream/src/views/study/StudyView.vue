  ...
  <div class="study-view">
    ...
    <div class="plan-sidebar">
      ...
      <div class="sidebar-actions">
        ...
        <el-button circle size="small" @click="openAddPlan">
      ...
      <div class="plan-list">
        ...
        >
          ...
          <el-button link type="danger" size="small" @click.stop="confirmDeletePlan(plan.id)">
    ...
    <div class="plan-detail" v-if="store.currentPlan">
      ...
      <div class="task-section">
        ...
        <div class="task-list">
          ...
          <el-empty v-if="store.tasks.length === 0" description="添加第一个任务" :image-size="60" />
        </div>
      </div>
    </div>
    ...
    <!-- 新建计划弹窗 -->
    <el-dialog v-model="showAddPlan" title="新建计划" width="460px">
      <el-form :model="planForm" label-width="80px">
        ...
        <el-form-item label="计划名称">
          <el-input v-model="planForm.title" :placeholder="`${catLabelOf(planForm.category)}计划名称`" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="addPlan" :disabled="!planForm.title.trim()">创建</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import type { PlanCategory } from '../../stores/study'
...
const planForm = reactive<{ title: string; goal: string; category: PlanCategory; color: string }>(
  { title: '', goal: '', category: 'study', color: '#0071e3' }
)
...
function openAddPlan() {
  ...
  planForm.category = cat as PlanCategory
  planForm.color = PLAN_CATEGORIES.find(c => c.value === cat)?.color ?? '#0071e3'
  planForm.title = ''
  planForm.goal = ''
...
async function addPlan() {
  ...
  await store.addPlan({ title: planForm.title, goal: planForm.goal, category: planForm.category, color: planForm.color })
...
async function confirmDeletePlan(id: string) {
  await ElMessageBox.confirm('确定删除该计划及所有任务吗？', '删除确认', {
...
.plan-card {
  ...
  border-radius: var(--radius-md); padding: 10px 10px 10px 0;
...
.plan-detail { flex: 1; overflow-y: auto; padding: 24px 28px; display: flex; flex-direction: column; gap: 20px; }
...
.detail-title-row h2 { font-size: 20px; font-weight: 700; color: var(--color-text); }
.detail-desc { font-size: 13px; color: var(--color-text-muted); }
...
.task-item {
  ...
  background: var(--color-bg-card); border: 1px solid var(--color-border);
  border-radius: var(--radius-sm); padding: 10px 12px; transition: all 150ms;
}
.task-item.done { opacity: 0.55; }
.task-title { flex: 1; font-size: 14px; color: var(--color-text); }
