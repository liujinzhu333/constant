...
import { reminderApi, type Reminder } from '../../utils/api'
...
async function loadList() {
  list.value = await reminderApi.list(activeTab.value)
}
...
async function snooze(r: Reminder) {
  ...
  await reminderApi.update(r.id, { status: "snoozed", remind_at: newTime })
  r.remind_at = newTime
  r.status = 'snoozed'
  // Web 环境暂不支持系统通知
}
...
async function dismiss(r: Reminder) {
  await reminderApi.update(r.id, { status: "dismissed" })
  r.status = 'dismissed'
...
async function submit() {
  ...
  await reminderApi.add({ source_type: 'custom', title: form.title, body: form.body, remind_at })
