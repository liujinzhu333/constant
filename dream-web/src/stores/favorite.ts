...
import { ref } from 'vue'
import { favoriteApi, type Favorite, type FavoriteType } from '../utils/api'
...
export const useFavoriteStore = defineStore('favorite', () => {
  ...
  async function add(data: {
    ...
    const created = await favoriteApi.add(JSON.parse(JSON.stringify(data)))
  ...
  async function update(id: string, data: Partial<Omit<Favorite, 'id' | 'created_at' | 'updated_at'>>) {
    // 用 JSON 序列化彻底解除 Vue 响应式 Proxy，避免 IPC 结构化克隆失败
    const updated = await favoriteApi.update(id, JSON.parse(JSON.stringify(data)))
  ...
  async function togglePin(item: Favorite) {
    ...
    await favoriteApi.pin(item.id, !!pinned)
